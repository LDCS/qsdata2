	...
	if true {
		...
		for _, kk := range append(append(append(strs, strsDf...), strsTgtd...), strsBlkid...) {
			if strings.HasPrefix(kk, "blkid:") {	// This would be true after handling all keys that are more fundamental than blkid
				kk	= kk[6:]
				if blkidDone[kk] { continue }
				if true { fmt.Println("qsdata2: unused blkid: kk=", kk) }
			}
			switch { 
			case strsDone[kk] : continue // skip tgtd whose key was seen as another regular object
			case tgtdDone[kk] : continue // skip tgtd whose key was seen from indirection of another object
			case dfCount[kk] > 0 : continue // skip df whose key was seen from indirection of another object
			}
			strsDone[kk]	= true
			md, _		:= mdmap[kk]
			scsi, _		:= scsimap[kk]
			dfArr, _	:= dfmap[kk];
			if len(dfArr) == 0 { dfArr = append(dfArr, nil) }
			for _, df0	:= range dfArr {
				...
				for _, parted := range partedArr {
					...
					if (parted != nil) && parted.Skip_ { continue }
					tgtd, _		:= tgtdmap[kk]
					if parted != nil {
					...
					if (df != nil) && (tgtd != nil) && (len(tgtd.Targetpath_) > 0) && (df.Name_ != tgtd.Targetpath_) { df = nil }
					if df == nil { df = qsdf.New() } else { dfCount[df.Name_] = dfCount[df.Name_] + 1 }
					...
					if opt.Verbose {
						...
						if parted == nil { ostr += "/parted=nil" } else { ostr += "/parted="+parted.Path_ }
						if tgtd == nil { ostr += "/tgtpath=nil" } else { ostr += "/tgtpath="+tgtd.Targetpath_ }
						ostr += ":"
					}
					switch {
					case (blkid == nil) && (parted != nil) && (blkidmap[parted.Path_] != nil)	: blkid = blkidmap[parted.Path_] 	// see if blkid info is available for the partition
					case (blkid == nil) && (scsi   != nil) && (blkidmap[scsi.Device_] != nil)	: blkid	= blkidmap[scsi.Device_]	// see if blkid info is available for the device
					case (blkid == nil) && (df     != nil) && (blkidmap[df.Name_]     != nil)	: blkid	= blkidmap[df.Name_]		// see if blkid info is available for the disk
					case (blkid == nil) && (md     != nil) && (blkidmap[md.Name_]     != nil)	: blkid	= blkidmap[md.Name_]		// see if blkid info is available for the disk
					}
					skiprow	:= false
					switch {
					...
					if skiprow { if opt.Verbose { ostr += genutil.StrTernary(skiprow, "!", "") } else { continue } }
					if blkid != nil { blkidDone[blkid.Devname_] = true }
